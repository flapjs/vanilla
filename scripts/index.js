...
function bind_double_click() {
  drawing.get_canvas().addEventListener("dblclick", (e) => {
    ...
    if (v) {
      ...
      const radius = Object.keys(graph).length
        ? Object.values(graph)[0].r
        : consts.DEFAULT_VERTEX_RADIUS;
...
function higher_order_drag_vertex(v) {
  ...
  let moved = false;
  drawing.get_canvas().addEventListener(
    "mouseup",
    () => {
      // additional event listener to hist.push_history
      if (moved) {
...
function higher_order_edge_animation(v) {
  ...
  const restore = () => {
    // helper to restore to the orignal canvas
  ...
  canvas.addEventListener(
    "mouseup",
    (e) => {
      // additional event listener to restore canvas and snap to vertex
      const [x, y] = drawing.event_position_on_canvas(e);
  ...
  return (e) => {
    ...
    restore();
    const [truncate_x, truncate_y] = linalg.normalize(
...
function bind_drag() {
  ...
  let edge_animation = consts.EMPTY_FUNCTION,
    drag_edge = consts.EMPTY_FUNCTION,
    drag_vertex = consts.EMPTY_FUNCTION;
  const canvas = drawing.get_canvas();
  canvas.addEventListener("mousedown", (e) => {
    ...
    if ((e.button === consts.RIGHT_BTN || e.ctrlKey) && clicked_vertex) {
      ...
      if (clicked_edge) {
        ...
        // vertex has lower priority than edge
  ...
  canvas.addEventListener("mouseup", (e) => {
    ...
    if (drop_vertex && drawing.over_trash(e)) {
      // delete vertex if dropped over trash
      graph_ops.delete_vertex(graph, drop_vertex);
    } else if (drop_edge && drawing.over_trash(e)) {
      // delete edge if dropped over trash
      graph_ops.delete_edge(graph, drop_edge);
    } else {
      // not dropped over trash can and release drag
...
function bind_plus_minus() {
  ...
  plus_button.addEventListener("click", () => {
    ...
    for (let i = 0; i < machine_inputs.children.length; i++) {
      if (
        machine_inputs.children.item(i).classList.contains("machine_input") &&
        machine_inputs.children.item(i).hidden
      ) {
...
function bind_run_input() {
  ...
  for (let i = 0; i < input_divs.length; i++) {
    ...
    run_btn.addEventListener("click", () => {
      ...
      computations[i] = compute.run_input(
        graph,
        menus.machine_type(),
        textbox.value
      ); // noninteractive computation
      ...
      input_divs[i].style.backgroundColor = accepted
        ? consts.ACCEPT_COLOR
        : consts.REJECT_COLOR;
  ...
  document
    ...
    .addEventListener("change", () => computations.fill(undefined));
}
...
function closeMenu() {
  if (homeToggle || machineToggle || saveToggle) {
    secondbar.style.transform = "translate(-20rem)";
    homeToggle = false;
    machineToggle = false;
    saveToggle = false;
  }
...
function toggleMenu(classname) {
  ...
  if (classname === "home" && !homeToggle) {
    clearMenu();
    secondbar.style.transform = "translate(0rem)";
